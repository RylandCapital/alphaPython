...
class alphaTerra(object):
    ...
    def pullMirrorHistory(self, start_date='2021-04-29', end_date=None):
        ...
        mirror_tokens = list(requests.get(
            'https://api.alphadefi.fund/info/tokendict').json()[0]['token'].keys())
    ...
    def mirrorSpreadStats(self, days_back):
        ...
        start_date = (dt.datetime.now()-dt.timedelta(
            days_back)).strftime('%m/%d/%y')
        end_date = (dt.datetime.now()+timedelta(days=1)).strftime("%m-%d-%Y")
        df = self.pullMirrorHistory(start_date, end_date)[2]
        df.rename(columns={'ticker':'symbol', 'value':'spread'},
                  inplace=True)
        
        

        statsy = df.groupby('symbol')['spread'].describe()[[ 'mean',
                                                    'std',
                                                    'min',
                                                    'max']]
    ...
    def masterAPR(self):
        ...
        ####################################################################
        '''calculate APRs'''
        df['isStableSwap'] = np.where(df['poolAddy'].isin(stableswaps), True,
                                      False)
        df['apr7d'] = np.where(df['isStableSwap']==False,
                                df['swaprate']*df['ustPoolVolume7d']/df['ustLiquidity']*52,
                                .00025*df['ustPoolVolume7d']/df['ustLiquidity']*52)
    ...
    def luna_staking(self):
        ...
        dashboard.columns = ['staking_return_daily', 'staking_return_annualized',
                             ...
                             'daily_block_rewards']
        dashboard['staking_return_daily_rank'] = dashboard[
            'staking_return_daily'].rank(pct=True)
        dashboard['staking_return_annualized_rank'] = dashboard[
            'staking_return_annualized'].rank(pct=True)
        dashboard['daily_transaction_volume_rank'] = dashboard[
            'daily_transaction_volume'].rank(pct=True)
        dashboard['daily_registered_accounts_rank'] = dashboard[
            'daily_registered_accounts'].rank(pct=True)
        ...
        mc = pd.DataFrame.from_dict(mc.json()).set_index('date').drop(
            '_id', axis=1).sort_index().dropna()
        mc.index = pd.to_datetime(mc.index).date
        
        #get ust price
        req = requests.get('https://eodhistoricaldata.com/api/eo' +
                           'd/{0}.CC?api_token={1}&period=d&fmt=json'.format(
                               'UST-USD', EOD_API))
        ...
        final['ustmc_1day_pct_change_rank'] = final['ustmc'].pct_change().rank(
            pct=True)
        final['ustmc_1day_pct_change_mean'] = final['ustmc'].pct_change().mean()
        final['ustmc_7day_pct_change'] = final['ustmc'].pct_change(
            7)
        final['ustmc_7day_pct_change_rank'] = final['ustmc'].pct_change(
            7).rank(pct=True)
        final['ustmc_7day_pct_change_mean'] = final['ustmc'].pct_change().mean()
        final['ustmc_1month_pct_change'] = final['ustmc'].pct_change(30)
        final['ustmc_1month_pct_change_rank'] = final['ustmc'].pct_change(
            30).rank(pct=True)
        final['ustmc_1month_pct_change_mean'] = final['ustmc'].pct_change(
            30).mean()
        final['ustmc_1year_pct_change'] = final['ustmc'].pct_change(
            365)
        final['ustmc_1year_pct_change_rank'] = final['ustmc'].pct_change(
            365).rank(pct=True)
        final['ustmc_1year_pct_change_mean'] = final['ustmc'].pct_change(
            365).mean()
        ...
        final['luna_ustmc_ratio_pct_rank'] = final['luna_ustmc_ratio'].rank(
            pct=True)
        ...
        final['luna_1week_returns_pctrank'] = final['luna_price'].pct_change(
            7).rank(pct=True)
        #luna 1 week returns
        final['luna_1month_returns_pctrank'] = final['luna_price'].pct_change(
            30).rank(pct=True)
        #luna 1 week returns
        final['luna_1year_returns_pctrank'] = final['luna_price'].pct_change(
            365).rank(pct=True)
        ...
        try:
            ...
            for i in ['BTC','SOL','ETH', 'LUNA']:
                ...
                cmc['symbol'] = i
                cmc['quote_dict'] = cmc['market_cap'].apply(
                    lambda x: json.loads(p.sub('\"', str(x))))
                cmc['Market Cap'] = cmc['quote_dict'].apply(
        ...
        for sym in ['BTC', 'ETH', 'SOL', 'LUNA']:
            for i in ['Market Cap', 'Market Cap Dominance']:
                temp = cmc[cmc['symbol']==sym][['date', i]].rename(columns={
                    i:'value', 'last_updated':'date'}).reset_index(drop=True)
        ...
        for sym in ['BTC', 'ETH', 'SOL']:
            for i in ['Market Cap']:
                temp = cmc[cmc['symbol']==sym][['date', i]].rename(columns={
                    i:'value'}).reset_index(drop=True)
                temp['date'] = pd.to_datetime(temp['date']).dt.date
                lunatemp = cmc[cmc['symbol']=='LUNA'][['date', i]].rename(columns={
                    i:'value'}).reset_index(drop=True)
                lunatemp['date'] = pd.to_datetime(lunatemp['date']).dt.date
                temp = temp.set_index('date').join(lunatemp.set_index(
                    'date'), on='date', rsuffix = '_luna')
