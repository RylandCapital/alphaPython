    ...
    with redirect_stdout(buf):
        ...
        try:
            ...
            liquidations[["bid_fee", "collateral_amount", "liquidator_fee", "repay_amount"]] = (
                liquidations[["bid_fee", "collateral_amount", "liquidator_fee", "repay_amount"]] / 1000000
            )
            liquidations["id"] = (
                liquidations["executed_height"].astype(str)
                + "-"
            ...
            if uts_now_seconds_left <= 30:
                ...
                uts_now = uts_now - dt.timedelta(seconds=liquidations["executed_at"].iloc[0].second)
            ...
            if uts_start_seconds_left <= 30:
                ...
                uts_start = uts_start - dt.timedelta(seconds=liquidations["executed_at"].iloc[-1].second)
            uts_start = int(dt.datetime.timestamp(uts_start))

            bluna = pd.DataFrame(
                requests.get(
                    r"https://api.coinhall.org/api/v1/charts/terra/candles?bars=33&from={0}&interval=1m&pairAddress=terra18r6rdnkgrg74zew3d8l9nhk0m4xanpeukw3e20&quoteAsset=uusd&to={1}".format(
                        uts_start, uts_now
                    )
                ).json()
            )
            bluna["datetime"] = (bluna.reset_index()["time"] / 1000).apply(lambda x: dt.datetime.utcfromtimestamp(x))
            beth = pd.DataFrame(
                requests.get(
                    r"https://api.coinhall.org/api/v1/charts/terra/candles?bars=33&from={0}&interval=1m&pairAddress=terra1c0afrdc5253tkp5wt7rxhuj42xwyf2lcre0s7c&quoteAsset=uusd&to={1}".format(
                        uts_start, uts_now
                    )
                ).json()
            )
            ...
            batom["datetime"] = (batom.reset_index()["time"] / 1000).apply(lambda x: dt.datetime.utcfromtimestamp(x))
            '''
            ...
            luna["datetime"] = (luna.reset_index()["time"] / 1000).apply(lambda x: dt.datetime.utcfromtimestamp(x))
            lunaust = pd.DataFrame(
                requests.get(
                    r"https://api.coinhall.org/api/v1/charts/terra/candles?bars=33&from={0}&interval=1m&pairAddress=terra1tndcaqxkpc5ce9qee5ggqf430mr2z3pefe5wj6&quoteAsset=uusd&to={1}".format(
                        uts_start, uts_now
                    )
                ).json()
            )
            ...
            liquidations["executed_at_round_timestamp"] = liquidations["executed_at"].apply(lambda x: minuteRound(x))
            ...
            liquidations["best_discount"] = liquidations["Discount_vs_UST_DEX_Price_at_Liquidation"].min()
            liquidations["average_discount"] = (
