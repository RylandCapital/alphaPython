...
'''define database'''        
client = pymongo.MongoClient(ALPHADEFI_MONGO,ssl=True,
...
def job():
        ...
        try: 
            ...
            json_data['price'] = json_data['prices'].apply(
                lambda x: x['price'] if x['price']==None else float(
                        x['price']))
            ...
            json_data['apr_short'] = json_data['statistic'].apply(
                lambda x:float(x['apr']['short']))
            ...
            for i in np.arange(len(json_data)):
                ...
                value = json_data.iloc[i]['price']/\
                json_data.iloc[i]['oralceprice']-1
                mydict = { "date": now,
                      "ticker": ticker,
                      "value": value }            
            ...
            if dt.datetime.now().minute == 0:                                  
                ...
                try:
                    supply = int(requests.get(
                    'https://fcd.terra.dev/v1/circulatingsupply/uusd').json())/1000000
                    mycol = db["ustMC"]
                    ticker = 'ust_circulating_supply'
                    mydict = { "date": now,
                          "ust_circulating_supply": supply }            
                    mycol.insert_one(mydict)
